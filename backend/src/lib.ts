...
export function setCsrfCookie(c: AppContext) {
	const token = crypto.randomUUID();
	c.header("Set-Cookie", `tai_csrf=${token}; Path=/; Secure; SameSite=None; Max-Age=86400`, { append: true });
	return token;
}

export function validateCsrf(c: AppContext): boolean {
	const method = c.req.method.toUpperCase();
	if (method === "GET" || method === "HEAD" || method === "OPTIONS") return true;

	const cookieHeader = c.req.header("cookie") || "";
	const csrfCookie = cookieHeader.split(";").map(s => s.trim()).find(s => s.startsWith("tai_csrf="));
...
function resolveSessionSecret(env: Env) {
	const secret = env.TAI_SESSION_SECRET || env.TAI_ENCRYPTION_KEY;
	if (!secret || secret.length < 32) {
		throw new Error("FATAL: TAI_SESSION_SECRET or TAI_ENCRYPTION_KEY must be set (min 32 chars). Server cannot start without a secure session secret.");
	}
	return secret;
}
...
export function getClientIp(c: AppContext) {
	// Priority 1: Cloudflare's verified header (cannot be spoofed when behind CF)
	const cfIp = c.req.header("cf-connecting-ip");
	if (cfIp && cfIp.trim()) return cfIp.trim();

	// Priority 2: X-Real-IP set by trusted reverse proxy (Nginx)
	const realIp = c.req.header("x-real-ip");
	if (realIp && realIp.trim()) return realIp.trim();

	// Priority 3: X-Forwarded-For — take LAST entry (closest trusted proxy)
...
export async function signupUser(env: Env, input: z.infer<typeof signupSchema>) {
	...
	const existing = await queryFirst<{ id: string; email_verified_at: string | null }>(env.DB, "SELECT id, email_verified_at FROM users WHERE lower(email) = lower(?) LIMIT 1", email);

	// Uniform response for all cases to prevent email enumeration
	const uniformResponse = (sent: boolean, token?: string, expiresAt?: string) => ({
		ok: true as const,
		verificationRequired: true,
		verificationTokenIssued: true,
		verificationEmailSent: sent,
		verificationToken: token ? maybeExposeToken(env, token) : undefined,
		verificationTokenExpiresAt: expiresAt,
	});

	if (existing) {
		if (existing.email_verified_at) {
			// Already verified — return same success shape, don't reveal account exists
			return uniformResponse(false);
		}
		// Exists but not verified — re-issue verification token
		...
		return uniformResponse(emailDelivery.sent, verification.token, verification.expiresAt);
	}
...
async function ensureBootstrapUsers(env: Env) {
	...
	bootstrapInitPromise = (async () => {
		...
		// App Store reviewer test account (only seeded when TAI_REVIEWER_PASSWORD is set)
		const reviewerPassword = envString(env, "TAI_REVIEWER_PASSWORD");
		if (reviewerPassword) {
			seedUsers.push({
...
export type GenerationStatus = "queued" | "reserving" | "processing" | "completed" | "failed" | "cancelled";
...
const FAL_MODEL_MAP: Record<string, { displayName: string; generationType: GenerationType; minTier: PlanTier; costUnits: number }> = {
	...
	"fal-ai/kling-video/v1.5/pro/text-to-video":                   { displayName: "TABAI Video",       generationType: "video",          minTier: "pro",     costUnits: 5000 },
	"fal-ai/veo3":                                                 { displayName: "TABAI Video Pro",   generationType: "video",          minTier: "power",   costUnits: 10000 },
	"fal-ai/kling-video/v1.5/pro/image-to-video":                  { displayName: "Animate Photo",     generationType: "image_to_video", minTier: "pro",     costUnits: 5000 },
};
...
// ═══════════════════════════════════════════════════════════════════════════
// PROFITABILITY LIMITS — Prices: Starter $9.99 | Pro $49.99 | Power $119.99
// After Apple 30%: Starter $6.99 | Pro $34.99 | Power $83.99
// All tiers profitable even at maximum usage on both monthly & yearly billing
// ═══════════════════════════════════════════════════════════════════════════

// ── fal.ai Generation Limits ──
export const FAL_TIER_LIMITS: Record<PlanTier, { imagesPerDay: number; imagesPerMonth: number; videosPerDay: number; videosPerMonth: number; monthlyUnitCap: number }> = {
	// 1 cost unit ≈ $0.000844
	free:    { imagesPerDay: 0,  imagesPerMonth: 0,   videosPerDay: 0, videosPerMonth: 0,  monthlyUnitCap: 0 },
	starter: { imagesPerDay: 2,  imagesPerMonth: 5,   videosPerDay: 0, videosPerMonth: 0,  monthlyUnitCap: 150 },
	pro:     { imagesPerDay: 3,  imagesPerMonth: 15,  videosPerDay: 1, videosPerMonth: 1,  monthlyUnitCap: 6000 },
	power:   { imagesPerDay: 8,  imagesPerMonth: 60,  videosPerDay: 2, videosPerMonth: 3,  monthlyUnitCap: 16000 },
};

// ── Per-Category Daily Chat Message Limits ──
// Controls OpenRouter cost per user per day. Category is computed from model metadata in mapModel().
export type ChatCategoryLimits = Record<ModelCategory, number>;
export const CHAT_TIER_LIMITS: Record<PlanTier, ChatCategoryLimits> = {
	free:    { "free-basic": 30,  "fast-everyday": 0,   "premium-creative": 0,   "premium-reasoning": 0,  "image-vision": 0,   "image-generation": 0, "hidden-internal": 0 },
	starter: { "free-basic": 100, "fast-everyday": 50,  "premium-creative": 0,   "premium-reasoning": 0,  "image-vision": 30,  "image-generation": 0, "hidden-internal": 0 },
	pro:     { "free-basic": 200, "fast-everyday": 100, "premium-creative": 40,  "premium-reasoning": 8,  "image-vision": 60,  "image-generation": 0, "hidden-internal": 0 },
	power:   { "free-basic": 500, "fast-everyday": 200, "premium-creative": 100, "premium-reasoning": 30, "image-vision": 100, "image-generation": 0, "hidden-internal": 0 },
};

/**
 * Check if user has remaining daily chat messages for this model's category.
 * Uses the rate_limits table (same schema as consumeRateLimit).
 */
export async function checkChatCategoryQuota(
	env: Env,
	userId: string,
	planTier: PlanTier,
	modelCategory: ModelCategory,
): Promise<{ allowed: true; used: number; limit: number } | { allowed: false; code: string; message: string; used: number; limit: number }> {
	const limits = CHAT_TIER_LIMITS[planTier];
	const dailyLimit = limits[modelCategory] ?? 0;

	if (dailyLimit <= 0) {
		const tierNeeded = modelCategory === "premium-reasoning" || modelCategory === "premium-creative" ? "Pro" : "Starter";
		return { allowed: false, code: "CHAT_CATEGORY_LOCKED", message: `This model requires a ${tierNeeded} plan or higher.`, used: 0, limit: 0 };
	}

	await ensureSecurityTables(env);

	// Use 86400s (24h) window aligned to UTC midnight
	const now = Date.now();
	const windowMs = 86400 * 1000;
	const dayStart = new Date();
	dayStart.setUTCHours(0, 0, 0, 0);
	const windowStartMs = dayStart.getTime();
	const scope = "chat_cat_daily";
	const identifier = `${modelCategory}:${userId}`;
	const identifierHash = await sha256Hex(`${scope}:${identifier}`);
	const bucketKey = `${scope}:${identifierHash}:${windowStartMs}`;

	const row = await queryFirst<{ count: number }>(
		env.DB,
		"SELECT count FROM rate_limits WHERE bucket_key = ? LIMIT 1",
		bucketKey,
	);
	...
	if (used >= dailyLimit) {
		return {
			allowed: false,
			code: "CHAT_DAILY_CATEGORY_LIMIT",
			message: `You've used all ${dailyLimit} daily messages for this model tier. Resets at midnight UTC.`,
			used,
			limit: dailyLimit,
		};
	}
...
/**
 * Increment the daily chat category counter after a successful stream.
 * Uses the same rate_limits table schema as consumeRateLimit.
 */
export async function incrementChatCategoryCount(
	env: Env,
	userId: string,
	modelCategory: ModelCategory,
): Promise<void> {
...
export async function checkFalBudget(
	...
	// Note: counts include 'reserving' status to prevent race conditions where
	// concurrent requests both pass the check before either inserts a reservation row.
	const activeStatuses = "('queued','reserving','processing','completed')";

	if (generationType === "image") {
		// Daily image check
		const dayRow = await queryFirst<{ cnt: number }>(
			env.DB,
			`SELECT COUNT(*) as cnt FROM generations WHERE user_id = ? AND generation_type = 'image' AND created_at >= ? AND status IN ${activeStatuses}`,
			userId, todayIso,
		);
		if ((dayRow?.cnt ?? 0) >= limits.imagesPerDay) {
		...
		if ((monthRow?.cnt ?? 0) >= limits.imagesPerMonth) {
			return { allowed: false, code: "FAL_MONTHLY_IMAGE_LIMIT", message: `Monthly image limit reached (${limits.imagesPerMonth}/month). Upgrade your plan for more.` };
		}
	} else {
		// video or image_to_video
		const dayRow = await queryFirst<{ cnt: number }>(
			env.DB,
			`SELECT COUNT(*) as cnt FROM generations WHERE user_id = ? AND generation_type IN ('video','image_to_video') AND created_at >= ? AND status IN ${activeStatuses}`,
			userId, todayIso,
		);
	...
	// Monthly cost unit cap — the primary profitability guard
	if (limits.monthlyUnitCap > 0) {
		const monthStart2 = new Date();
		monthStart2.setUTCDate(1);
		monthStart2.setUTCHours(0, 0, 0, 0);
		const monthIso2 = monthStart2.toISOString();
		const unitRow = await queryFirst<{ total: number }>(
			env.DB,
			`SELECT COALESCE(SUM(estimated_cost_units), 0) as total FROM generations WHERE user_id = ? AND created_at >= ? AND status IN ${activeStatuses}`,
			userId, monthIso2,
		);
		const currentUnits = unitRow?.total ?? 0;
		const modelCost = FAL_MODEL_MAP[modelId]?.costUnits ?? 0;
		if (currentUnits + modelCost > limits.monthlyUnitCap) {
			return { allowed: false, code: "FAL_MONTHLY_UNIT_LIMIT", message: `Monthly generation budget exhausted. Upgrade your plan for more.` };
		}
	}
...
export type FalQuotaSnapshot = {
	planTier: PlanTier;
	images: { usedToday: number; limitPerDay: number; usedThisMonth: number; limitPerMonth: number };
	videos: { usedToday: number; limitPerDay: number; usedThisMonth: number; limitPerMonth: number };
	monthlyUnits: { used: number; limit: number };
	generationEnabled: boolean;
};

export async function getFalQuotaSnapshot(env: Env, userId: string, planTier: PlanTier): Promise<FalQuotaSnapshot> {
	const limits = FAL_TIER_LIMITS[planTier];
	const generationEnabled = limits.imagesPerDay > 0 || limits.videosPerDay > 0;
	...
	// Include 'reserving' status to match budget check (prevents showing stale counts during concurrent requests)
	const activeFilter = "AND status IN ('queued','reserving','processing','completed')";
	const [imagesToday, imagesMonth, videosToday, videosMonth, monthlyUnits] = await Promise.all([
		queryFirst<{ cnt: number }>(env.DB,
			`SELECT COUNT(*) as cnt FROM generations WHERE user_id = ? AND generation_type = 'image' AND created_at >= ? ${activeFilter}`,
			userId, todayIso),
		queryFirst<{ cnt: number }>(env.DB,
			`SELECT COUNT(*) as cnt FROM generations WHERE user_id = ? AND generation_type = 'image' AND created_at >= ? ${activeFilter}`,
			userId, monthIso),
		queryFirst<{ cnt: number }>(env.DB,
			`SELECT COUNT(*) as cnt FROM generations WHERE user_id = ? AND generation_type IN ('video','image_to_video') AND created_at >= ? ${activeFilter}`,
			userId, todayIso),
		queryFirst<{ cnt: number }>(env.DB,
			`SELECT COUNT(*) as cnt FROM generations WHERE user_id = ? AND generation_type IN ('video','image_to_video') AND created_at >= ? ${activeFilter}`,
			userId, monthIso),
		queryFirst<{ total: number }>(env.DB,
			`SELECT COALESCE(SUM(estimated_cost_units), 0) as total FROM generations WHERE user_id = ? AND created_at >= ? ${activeFilter}`,
			userId, monthIso),
	]);
	...
	return {
		planTier,
		images: {
			usedToday: imagesToday?.cnt ?? 0,
			limitPerDay: limits.imagesPerDay,
			usedThisMonth: imagesMonth?.cnt ?? 0,
			limitPerMonth: limits.imagesPerMonth,
		},
		videos: {
			usedToday: videosToday?.cnt ?? 0,
			limitPerDay: limits.videosPerDay,
			usedThisMonth: videosMonth?.cnt ?? 0,
			limitPerMonth: limits.videosPerMonth,
		},
		monthlyUnits: {
			used: monthlyUnits?.total ?? 0,
			limit: limits.monthlyUnitCap,
		},
		generationEnabled,
	};
}
...
export async function submitFalGeneration(
	...
	// ── Atomic reservation: insert row BEFORE calling fal.ai to prevent race conditions ──
	// This ensures concurrent requests see each other's reservations when checking budget limits.
	await exec(
		env.DB,
		`INSERT INTO generations (id, user_id, chat_id, fal_request_id, provider, model_id, model_display_name, generation_type, status, prompt, negative_prompt, parameters, estimated_cost_units, created_at)
		 VALUES (?, ?, ?, ?, 'fal', ?, ?, ?, 'reserving', ?, ?, ?, ?, ?)`,
		id,
		userId,
		chatId ?? null,
		"pending", // placeholder until fal.ai responds
		modelId,
	...
	let res: Response;
	try {
		res = await fetch(`https://queue.fal.run/${modelId}`, {
			method: "POST",
			headers: {
		...
		// Network error — release the reservation
		await exec(env.DB, "UPDATE generations SET status = 'cancelled' WHERE id = ?", id);
		return { error: "Failed to connect to generation service." };
	}

	if (!res.ok) {
		const text = await res.text();
		// Release the reservation on fal.ai error
		await exec(env.DB, "UPDATE generations SET status = 'cancelled' WHERE id = ?", id);
		return { error: `fal.ai error (${res.status}): ${text.slice(0, 200)}` };
	}

	const result = await res.json<{ request_id: string; status_url?: string; response_url?: string }>();

	// Store fal.ai canonical URLs alongside user params so status/result checks use them
	const storedParams: Record<string, unknown> = { ...params };
	if (result.status_url) storedParams._fal_status_url = result.status_url;
	if (result.response_url) storedParams._fal_response_url = result.response_url;

	// Promote reservation to active queued state with real fal request ID
	await exec(
		env.DB,
		"UPDATE generations SET fal_request_id = ?, status = 'queued', parameters = ? WHERE id = ?",
		result.request_id,
...
export async function checkFalStatus(
	...
	falRequestId: string,
	modelId: string,
	storedStatusUrl?: string | null,
): Promise<{ status: string; queuePosition?: number; responseUrl?: string }> {
	const apiKey = envString(env, "FAL_AI_API_KEY");
	const url = storedStatusUrl || `https://queue.fal.run/${modelId}/requests/${falRequestId}/status`;
...
export async function getFalResult(
	...
	falRequestId: string,
	modelId: string,
	storedResponseUrl?: string | null,
): Promise<{ data: Record<string, unknown> } | { error: string }> {
	const apiKey = envString(env, "FAL_AI_API_KEY");
	const url = storedResponseUrl || `https://queue.fal.run/${modelId}/requests/${falRequestId}`;
	const res = await fetch(url, {
		headers: { Authorization: `Key ${apiKey}` },
	});
	if (!res.ok) {
...
export async function cancelGeneration(env: Env, userId: string, generationId: string): Promise<boolean> {
	...
	if (!row) return false;
	if (row.status !== "queued" && row.status !== "reserving" && row.status !== "processing") return false;
