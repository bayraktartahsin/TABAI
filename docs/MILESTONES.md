# TABAI — Master Product Scope & Milestone Tracker

> **Owner:** Volkan Bayraktar
> **Product:** TABAI Multi-AI Assistant (Web + iOS + Android)
...
Browser/iOS/Android → ai.gravitilabs.com
  → Nginx → static assets (Next.js 14 export)
  → /api/* → backend (Node.js + Hono + SQLite)
...
### Starter Tier ($6.99/mo) — Fast & Everyday + Vision
...
### Pro Tier ($29.99/mo) — Premium Reasoning + Creative
...
### Power Tier ($79.99/mo) — Everything Unlocked
...
**Reference:** See `docs/fal-ai-integration.md` for full details, autopilot prompts, and pricing.
...
### Web Key Files
```
web/components/chat/ChatScreen.tsx — Chat interface
web/components/ui/AppShell.tsx — Main orchestrator
web/components/ui/Sidebar.tsx — Navigation sidebar
web/components/ui/LoginScreen.tsx — Auth UI
web/components/panels/PricingPage.tsx — Pricing
web/components/legal/LegalDocumentPage.tsx — Legal template
web/app/privacy/page.tsx — Privacy policy (✅ fixed)
web/app/terms/page.tsx — Terms (✅ fixed)
web/app/acceptable-use/page.tsx — AUP (✅ fixed)
web/app/subscription/page.tsx — Billing (✅ fixed)
web/app/support/page.tsx — Support (✅ fixed)
web/lib/design-system/tokens.ts — Theme tokens
web/lib/tai/client.ts — API client
```

### Backend Key Files
```
backend/src/index.ts — Route registration, static assets, CORS
backend/src/lib.ts — All business logic (4000+ lines)
backend/src/endpoints/tai.ts — 42 endpoint handlers
backend/src/types.ts — Zod schemas
backend/wrangler.jsonc — Cloudflare config (now includes static assets)
backend/migrations/ — D1 schema (8 migrations)
```
