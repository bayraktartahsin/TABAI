...
export class AuthAppleRoute extends OpenAPIRoute {
	...
	async handle(c: AppContext) {
		...
		if (!result.user) {
			return jsonError(c, 400, "No email available. This appears to be a new account — please ensure email scope is requested on first sign-in.");
		}
...
export class AuthSignupRoute extends OpenAPIRoute {
	...
	async handle(c: AppContext) {
		...
		if (!result.ok) {
			return jsonError(c, 409, result.error, result.code);
		}
		applyCors(c);
		return c.json({
			success: true,
...
export class GenerateStatusRoute extends OpenAPIRoute {
	...
	async handle(c: AppContext) {
		...
		if (gen.status === "queued" || gen.status === "processing") {
			const falStatus = await checkFalStatus(c.env, gen.falRequestId, gen.modelId);
			...
			if (newStatus === "completed") {
				const resultData = await getFalResult(c.env, gen.falRequestId, gen.modelId);
